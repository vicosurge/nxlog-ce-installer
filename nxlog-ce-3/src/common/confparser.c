...
/**
 * Gets last nx_directive_t of directive chain which has directive==key
 *
 * @param conf head of directives chain
 * @param key
 * @return
 */
const nx_directive_t *nx_cfg_lookup(const nx_directive_t *conf, const char *key)
{
    ASSERT(conf);
    ASSERT(key);
    const nx_directive_t *curr = conf;
    uint32_t found_directives = 0;
    const nx_directive_t *last_directive = NULL;
    ...
    if ( found_directives == FALSE )
    {
	return ( NULL );
    }

    if ( found_directives > 1 )
    {
	log_warn("directive '%s' has %d occurrences in config file; last one is used ('%s')",
		 key, found_directives, last_directive->args);
    }
...
/**
 * Will only set the bool value if conf is not NULL
 */
void nx_cfg_boolean(const nx_directive_t *conf, boolean *value)
{
    if ( conf == NULL )
    {
	return;
...
void nx_cfg_get_boolean(const nx_directive_t *conf,
			...
			boolean *value)
{
    nx_cfg_boolean(nx_cfg_lookup(conf, key), value);
}

char *nx_cfg_unquote_string(const nx_directive_t *conf, apr_pool_t *pool)
{
    if ( conf == NULL )
    {
	return NULL;
    ...
    size_t len, lastchar;
    const char *str = conf->args;
    char *result = NULL;

    if ( str != NULL )
    {
	result = apr_palloc(pool, (apr_size_t) strlen(str)+1);
	strcpy(result, str);
	...
	if ( (result[0] == '"') || (result[0] == '\'') )
	{
	    ...
	    {
		if ( result[lastchar] != '"' )
		{ // malformed input "/path/file.txt?
		    log_warn("Quotation is possibly invalid in %s at %s:%d",
			     str, conf->filename, conf->line_num);
		}
		else
		{
		    lastchar = nx_string_unescape_c(result) - 1;
		}
	    }
	    else
	    { // first char is single quote
		if ( result[lastchar] != '\'' )
		{ // malformed input '/path/file.txt?
		    log_warn("Quotation is possibly invalid %s at %s:%d",
			     str, conf->filename, conf->line_num);
		}
	    }
	...
	else if ( (result[lastchar] == '"') || (result[lastchar] == '\'') )
	{ // malformed input /path/file.txt" or /path/file.txt'
	    log_warn("Quotation is possibly invalid %s at %s:%d",
		     str, conf->filename, conf->line_num);
	    result[lastchar] = '\0';
	}
...
const char *nx_cfg_get_string_unquoted(const nx_directive_t *conf,
				       const char *key,
				       apr_pool_t *pool)
{
    return  nx_cfg_unquote_string(nx_cfg_lookup(conf, key), pool);
}

/**
 * Throws an exception in case of hex value read error
 *
 * @param conf
 * @param hex_allowed hexadecimal character value allowed
 * @return character value of the directive OR '\0' in case of invalid format character value
 */
char nx_cfg_get_char(const nx_directive_t *conf, boolean hex_allowed)
{
    ASSERT(conf != NULL);
	...
	case 1:
	    ...
	    break;
	default:
	    if ( (hex_allowed == TRUE) && (strncasecmp(str, "0X", 2) == 0) )
	    {
