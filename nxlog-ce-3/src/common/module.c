...
void *nx_ctx_data_get(nx_ctx_t * ctx, const char * key)
{
    nx_ctx_data_t *cur;
    void *retval = NULL;
...
void nx_ctx_data_set(nx_ctx_t * ctx, const char * key, void * data)
{
    nx_ctx_data_t *new;
    nx_ctx_data_t *cur;
    ...
    new = apr_palloc(ctx->pool, sizeof(nx_module_data_t));
    new->key = apr_pstrdup(ctx->pool, key);
    new->data = data;
    new->next = ctx->data;
    ctx->data = new;
}
...
apr_status_t nx_module_input_fill_buffer_from_socket(nx_module_input_t *input)
{
    ...
    from = nx_module_input_data_get(input, NX_MODULE_INPUT_CONTEXT_RECV_FROM);
    if ( from == NULL )
    {
	from = apr_pcalloc(input->pool, sizeof(apr_sockaddr_t));
	nx_module_input_data_set(input, NX_MODULE_INPUT_CONTEXT_RECV_FROM, from);
    }
...
boolean nx_module_add_logdata_input(nx_module_t *module,
...
void nx_module_shutdown_self(nx_module_t *module)
{
    ...
    // cleanup in module->data list
    nx_module_data_cleanup(module);
...
void *nx_module_data_get(nx_module_t *module, const char *key)
{
    ...
    CHECKERR(apr_thread_mutex_lock(module->data_mutex));
    ...
    CHECKERR(apr_thread_mutex_unlock(module->data_mutex));
    return ( retval );
}
 ...
 */
void nx_module_data_set(nx_module_t *module, const char *key, void *data, void(*cleanup_fn)(void *))
{
    ...
    new->data = data;
    new->data_cleanup_fn = cleanup_fn;
...
// do cleanup on nx_module_data_t.data if there is cleanup fn defined; called from module shutdown!
void nx_module_data_cleanup(nx_module_t *module)
{
    ASSERT(module != NULL);
    nx_module_data_t *cur;
    CHECKERR(apr_thread_mutex_lock(module->data_mutex));
    ...
    while ( cur != NULL )
    {
	if ( cur->data_cleanup_fn != NULL )
	{
	    cur->data_cleanup_fn(cur->data);
	    cur->data = NULL;
	}
	cur = cur->next;
    }
...
/**
 * Process unprocessed buffers / destroys stream contexts
 *
 * @param input
 * @return
 */
void nx_module_input_flush(nx_module_input_t *input)
{
    if ( (input == NULL) || (input->inputfunc == NULL) )
    {
	return;
    }
    ...
    // TODO: is it not needed any longer ????
    // we need to flush xm_multiline and other extension's buffers to avoid loosing data
    if ( (input != NULL) && (input->inputfunc != NULL) && (input->inputfunc->flush != NULL) )
    {
	while ( (record = input->inputfunc->flush(input, input->inputfunc->data)) != NULL )
	{
	    nx_module_add_logdata_input(input->module, input, record);
	}
    }
}
