		      ...
		      const nx_expr_t *right)
{
  ...
  match_global:
	...
	for ( i = 0; i < result; i++ )
	{
	    eval_ctx->captured[i] = nx_string_create_throw(subject + ovector[i * 2],
	...
	if ( replacement != NULL )
	{
	    ...
	    nx_string_ensure_size_throw(fieldval.string,
...
nx_expr_t *nx_expr_new_string(nx_expr_parser_t *parser, const char *str)
{
    ...
    {
	expr->value.string = nx_string_create_throw(str, -1);
    }
