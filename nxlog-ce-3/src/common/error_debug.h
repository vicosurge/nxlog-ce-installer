...
#define TRIAL_CHECK() \
    { \
        if ( apr_time_now() > BUILDTIME * APR_USEC_PER_SEC + (BUILD_TRIAL * 24 * 60 * 60 * APR_USEC_PER_SEC) ) \
        { \
	    log_error("This NXLog version has expired."); \
	    _nx_error_dialog("This NXLog version has expired"); \
	    exit(1); \
        } \
    }

#else //BUILD_TRIAL not defined
#define TRIAL_CHECK()
#endif
