...
#define NX_MODULE_INPUT_CONTEXT_FILENAME              "nx_module_filename"
#define NX_MODULE_INPUT_CONTEXT_RECV_FROM             "nx_module_recv_from"
#define NX_MODULE_INPUT_CONTEXT_RECV_FROM_STR         "nx_module_recv_from_str"
...
typedef void (nx_module_input_clean_t)(nx_module_input_t *input, void *data);
typedef struct nx_module_input_func_decl_t nx_module_input_func_decl_t;
struct nx_module_data_t
{
    const char *key;
    void *data;
    void  (*data_cleanup_fn)(void *data);
...
struct nx_module_input_func_decl_t
{
    ...
    nx_module_input_clean_t *clean; ///< clean up
    void *data;
};
struct nx_module_input_t
{
    ...
    int			incomplete_len;  ///< consumed bytes from buffer but still resides in parser ctx (not used for generating record)
    void		*ctx;		///< parser context or user defined context
...
struct nx_module_t
{
    ...
    apr_thread_mutex_t	*data_mutex;    ///< mutex for 'data' field
    union 
    {
