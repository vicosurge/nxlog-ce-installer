...
typedef enum nx_event_type_t
{
    ...
    NX_EVENT_CONFIG_CACHE_FLUSH,
    NX_EVENT_LAST_DUMMY_EVENT      // used as number of event types
} nx_event_type_t;
...
struct nx_event_t
{
    ...
    apr_time_t			time;
    void			*data;
    int				subtype;  ///> module specific type
    int				priority;
    nx_job_t			*job;
};
...
nx_event_t *nx_event_add(nx_event_t *event);
