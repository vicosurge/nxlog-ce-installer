...
nx_ctx_t *nx_ctx_new()
{
    ...
    ctx->escape_glob_patterns = FALSE;
...
#ifdef WIN32
#define log_registry_error(statuscode, fmt, args...) log_aprerror((apr_status_t) APR_FROM_OS_ERROR(statuscode), fmt, ## args)
#define win_err_log(fmt, args...) log_aprerror((apr_status_t) APR_FROM_OS_ERROR(GetLastError()), fmt, ## args)


static char* get_win32_installdir(apr_pool_t *pool)
{
    ASSERT(pool != NULL);
    HKEY regkey = NULL;
    DWORD regtype = 0;
    ...
    char *ret = NULL;
    LONG ret_regopen;
    LONG ret_regqueryval;

    memset(buf, 0, (size_t) bufsize);

    ret_regopen = RegOpenKey(HKEY_LOCAL_MACHINE, "SOFTWARE\\nxlog", &regkey);
    if ( ret_regopen == ERROR_SUCCESS )
    {
	ret_regqueryval = RegQueryValueExA(regkey, "installdir", 0, &regtype, (unsigned char *) buf, &bufsize);
	if ( ret_regqueryval == ERROR_SUCCESS )
	{
	    if ( regtype == REG_SZ && bufsize > 0 )
	    {
		DWORD i = 1;
		if ( buf[bufsize - 1] == '\x0' )
		{
		...
		if ( buf[bufsize - i] == '\\' || buf[bufsize - i] == '/' )
		{
		    // remove separator
		    buf[bufsize - i] = '\x0';
		}
	...
	{
	    log_registry_error(ret_regqueryval, "RegQueryValueExA failed at 'HKLM\\SOFTWARE\\nxlog\\installdir' with error code %ld", ret_regqueryval);
	}
	RegCloseKey(regkey);
    }
    else
    {
	log_registry_error(ret_regopen, "RegOpenKey failed at 'HKLM\\SOFTWARE\\nxlog' with error code %ld", ret_regopen);
    }

    if ( ret == NULL )
    {
	// in the last resort we use process executable path as installdir
	DWORD len = GetModuleFileNameA(NULL, buf, bufsize / sizeof (CHAR));
	if ( (len > 0) && (len < bufsize / sizeof (CHAR)) )
	{
	    char *last_sep = strrchr(buf, '\\');
	    if ( last_sep != NULL )
	    {
		// terminate filepath before executable name
		*last_sep = '\x0';
		ret = apr_pstrdup(pool, buf);
	    }
	}
	...
	{
	    win_err_log("GetModuleFileNameA() failed");
	}
    }
...
void nx_ctx_parse_cfg(nx_ctx_t *ctx, const char *cfgpath)
{
...
#ifdef WIN32
    tmp_pool = nx_pool_create_child(ctx->pool);
    ASSERT(tmp_pool != NULL);
    char *installdir = get_win32_installdir(tmp_pool);

    if ( installdir != NULL )
    {
	size_t installdir_size = strlen(installdir);
	if ( installdir_size > 0 && installdir[installdir_size - 1] == NX_DIR_SEPARATOR[0] )
	{
	    installdir[installdir_size - 1] = '\x0';
	}
    }
    else
    {
	log_info("NXLog couldn't get installation directory due to improper installation or registry issues");
    }
#endif
    ...
    {
	ctx->moduledir = nx_cfg_get_string_unquoted(ctx->cfgtree, "moduledir", ctx->pool);
    }
    else
    {
#ifdef WIN32
	if ( installdir != NULL )
	{
	    ctx->moduledir = apr_pstrcat(ctx->pool, installdir, NX_DIR_SEPARATOR"modules", NULL);
	    ASSERT(ctx->moduledir != NULL);
	}
	else
	{
	    nx_conf_error(ctx->cfgtree, "`installdir` was not found in the registry. Please install NXLog properly or specify 'Moduledir' in the configuration file.");
	}
#else
    ...
    {
	ctx->cachedir = nx_cfg_get_string_unquoted(ctx->cfgtree, "cachedir", ctx->pool);
    }
    else
    {
    ...
    {
	ctx->rootdir = nx_cfg_get_string_unquoted(ctx->cfgtree, "rootdir", ctx->pool);
    }

    if ( nx_cfg_get_value(ctx->cfgtree, "spooldir") != NULL )
    {
	ctx->spooldir = nx_cfg_get_string_unquoted(ctx->cfgtree, "spooldir", ctx->pool);
    }
    ...
    nx_cfg_get_boolean(ctx->cfgtree, "escapeglobpatterns", &(ctx->escape_glob_patterns));
...
void nx_ctx_init_logging(nx_ctx_t *ctx)
{
    ...
    logfilename = nx_cfg_get_string_unquoted(ctx->cfgtree, "logfile", ctx->pool);
    if ( logfilename == NULL )
    {
	return;
    }
