...
static void om_udp_write(nx_module_t *module)
{
    ...
    {
	log_debug("module %s is not running, not sending any more data", module->name);
	return;
    }
...
static void om_udp_config(nx_module_t *module)
{
	...
	else if ( strcasecmp(curr->directive, "localport") == 0 )
	{
	    if ( omconf->localport != 0 )
	    {
		nx_conf_error(curr, "LocalPort is already defined");
	    }
	    if ( sscanf(curr->args, "%u", &port) != 1 )
	    {
		nx_conf_error(curr, "invalid port: %s", curr->args);
	    }
	    omconf->localport = (apr_port_t) port;
	}
...
static void om_udp_connect(nx_module_t *module)
{
	...
	if ( omconf->localport != 0 )
	{
	    ...
	    CHECKERR_MSG(apr_sockaddr_info_get(&sa_local, "0.0.0.0", APR_INET, omconf->localport, 0, pool),
			 "apr_sockaddr_info failed for %s:%d", "0.0.0.0", omconf->port);
	    CHECKERR_MSG(apr_socket_bind(omconf->sock, sa_local),
			 "couldn't bind udp socket to %s:%d", "0.0.0.0", omconf->localport);
	}
