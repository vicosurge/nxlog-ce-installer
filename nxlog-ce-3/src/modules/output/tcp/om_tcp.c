...
static void om_tcp_stop(nx_module_t *module)
{
	...
	{
	    // don't send partial data after re-connect
	    module->output.buflen += module->output.bufstart;
	}
...
static void om_tcp_config(nx_module_t *module)
{
    ...
    module->output.pool = nx_pool_create_child(module->pool);
}
