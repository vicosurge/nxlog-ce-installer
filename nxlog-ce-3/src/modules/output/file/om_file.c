...
#define OM_FILE_MAX_RETRY_INTERVAL 200
...
static void om_file_write(nx_module_t *module)
{
	...
	if ( module->output.buflen > 0 )
	{
	    ...
	    rv = apr_file_write(omconf->file, module->output.buf + module->output.bufstart, &nbytes);
	    if ( rv != APR_SUCCESS )
	    {
		if ( APR_STATUS_IS_EAGAIN(rv) || APR_STATUS_IS_TIMEUP(rv) )
		{
		    nx_module_pollset_add_file(module, omconf->file, APR_POLLOUT | APR_POLLHUP);
		    nx_module_add_poll_event(module);
		    done = TRUE;
		}
		else
		{
		    throw(rv, "apr_file_write failed");
		}
	    }
	    else
	    {
...
static void om_file_add_retry_event(nx_module_t *module)
{
    nx_event_t *event;
    nx_om_file_conf_t *omconf;
    ...
    if ( omconf->retry > OM_FILE_MAX_RETRY_INTERVAL )
    {
	omconf->retry = OM_FILE_MAX_RETRY_INTERVAL;
    }

    log_info("retrying write in %d seconds", omconf->retry);

    event = nx_event_new();
    event->module = module;
    event->delayed = TRUE;
    event->type = NX_EVENT_RECONNECT;
    event->time = apr_time_now() + APR_USEC_PER_SEC * omconf->retry;
...
static void io_err_handler(nx_module_t *module, nx_exception_t *e)
{
    ...
    om_file_stop(module);
    om_file_add_retry_event(module);
    rethrow(*e);
}
...
static void om_file_event(nx_module_t *module, nx_event_t *event)
{
    ...
    switch ( event->type )
    {
	    ...
	    // This is a retry - not a reconnect
	...
	case NX_EVENT_DATA_AVAILABLE:
		...
		if (omconf->retry != 0)
		{
		    log_debug("write succeeded for module '%s' ", module->name);
		    omconf->retry = 0;
		}
	    }
	    catch(e)
