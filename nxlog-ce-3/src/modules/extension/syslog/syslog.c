				...
				const char *hostend)
{
	...
	hostname->string = nx_string_create_throw(hoststart, len);
			       ...
			       const char *end)
{
	...
	len = (int) (end - start);
	application->string = nx_string_create_throw(start, len);
			      ...
			      const char *end)
{
	...
	ASSERT(procid != NULL);
	len = (int) (end - start);
	procid->string = nx_string_create_throw(start, len);
			     ...
			     const char *end)
{
	...
	ASSERT(msgid != NULL);
	len = (int) (end - start);
	msgid->string = nx_string_create_throw(start, len);
			       ...
			       const char *end)
{
	...
	ASSERT(message != NULL);
	message->string = nx_string_create_throw(start, len);
					  ...
					  const char *string)
{
	...
	for ( i = 0; *ptr != '\0'; i++, ptr++ )
	{
		...
		{
		    ptr++;
		    nx_string_append_throw(param_value, ptr, 1);
		}
	    }
	    else if ( *ptr == '"' )
	    {
...
static void nx_syslog_add_structured_data(nx_logdata_t *logdata)
{
    ...
    nx_string_append_throw(sd, "[NXLOG@14506 ", -1);
	...
	if ( (ignore == FALSE) && (field->value->defined == TRUE) )
	{
	    if ( cnt > 0 )
	    {
		nx_string_append_throw(sd, " ", 1);
	    }
	    cnt++;
	    
	    sdlen = sd->len;
	    nx_string_append_throw(sd, field->key, -1);
		...
		for ( i = 0; i < (int) field->value->string->len; i++ )
		{
		    ...
		    nx_string_append_throw(sd, field->value->string->buf + i, 1);
		}
	    }
	    else
	    {
		value = nx_value_to_string(field->value);
		nx_string_append_throw(sd, value, -1);
		free(value);
	    }
	    nx_string_append_throw(sd, "\"", 1);
	}
...
void nx_logdata_to_syslog_rfc5424(nx_logdata_t *logdata, boolean utc)
{
    int pri = 0;
    nx_value_t hostname;
    nx_value_t timestamp;
    ...
    if ( (nx_logdata_get_field_value(logdata, "EventTime", &timestamp) == TRUE) &&
	 (timestamp.type == NX_VALUE_TYPE_DATETIME) && (timestamp.defined == TRUE) )
    {
	nx_date_to_rfc5424(tmpstr, sizeof(tmpstr), utc, timestamp.datetime);
    }
    else
    {
	nx_date_to_rfc5424(tmpstr, sizeof(tmpstr), utc, apr_time_now());
    }
