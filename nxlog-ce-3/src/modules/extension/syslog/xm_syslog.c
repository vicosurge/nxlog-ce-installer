						  ...
						  void *data UNUSED)
{
	...
	nx_string_append_throw(incomplete_logdata, input->buf + input->bufstart, appendlen);
	...
	if ( got_space != TRUE )
	{
	    ...
	    input->ctx = (void * ) nx_string_create_throw(input->buf + input->bufstart, input->buflen);
	    input->bufstart = 0;
	    input->buflen = 0;
	    return ( NULL );
	}
