					 ...
					 nx_expr_list_t *args)
{
	...
	{ // we have wildcards, expand it
	    ...
	    if ( idx == NULL )
	    {
		dirname = nx_string_create_throw("."NX_DIR_SEPARATOR, -1);
	    }
	    else
	    {
		dirname = nx_string_create_throw(filename, (int) (idx + 1 - filename));
		filename = idx + 1;
	    }
				       ...
				       nx_value_t *args)
{
    ...
    retval->string = nx_string_new_size_throw((size_t) finfo.size);
					   ...
					   nx_value_t *args)
{
    ...
    if ( idx == NULL )
    {
	retval->string = nx_string_create_throw(filename, -1);
	return;
    }

    retval->string = nx_string_create_throw(idx + 1, -1);
}
					  ...
					  nx_value_t *args)
{
    ...
    retval->string = nx_string_create_throw(filename, (int) (idx - filename));
}
				       ...
				       nx_value_t *args)
{
    ...
    {
	case APR_REG:
	    retval->string = nx_string_create_throw("FILE", -1);
	    break;
	case APR_DIR:
	    retval->string = nx_string_create_throw("DIR", -1);
	    break;
	case APR_CHR:
	    retval->string = nx_string_create_throw("CHAR", -1);
	    break;
	case APR_BLK:
	    retval->string = nx_string_create_throw("BLOCK", -1);
	    break;
	case APR_PIPE:
	    retval->string = nx_string_create_throw("PIPE", -1);
	    break;
	case APR_LNK:
	    retval->string = nx_string_create_throw("LINK", -1);
	    break;
	case APR_SOCK:
	    retval->string = nx_string_create_throw("SOCKET", -1);
	    break;
	case APR_NOFILE:
	case APR_UNKFILE:
	default:
	    retval->string = nx_string_create_throw("UNKNOWN", -1);
	    break;
    }
					  ...
					  nx_value_t *args UNUSED)
{
    ...
    retval->defined = TRUE;
    retval->string = nx_string_create_throw(tmpdir, -1);
    apr_pool_destroy(pool);
}
