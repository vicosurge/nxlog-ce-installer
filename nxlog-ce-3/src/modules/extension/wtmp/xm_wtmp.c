...
static nx_logdata_t* xm_wtmp_parser(struct utmp *u)
{
    ...
    nx_string_append_throw(raw_input_str, "    ", 4);
    nx_string_append_throw(raw_input_str, u->ut_user, -1);
    nx_string_append_throw(raw_input_str, "    ", 4);

    if (strlen(u->ut_line) >= 3)
    {
        nx_logdata_set_string(logdata, "Device", u->ut_line);
        nx_string_append_throw(raw_input_str, u->ut_line, -1);
        nx_string_append_throw(raw_input_str, "    ", 4);
    }
    else
    {
        nx_logdata_set_string(logdata, "Device", "no device");
        nx_string_append_throw(raw_input_str, "no device", -1);
        nx_string_append_throw(raw_input_str, "    ", 4);
    }
    ...
    nx_string_append_throw(raw_input_str, type, (int) strlen(type));
