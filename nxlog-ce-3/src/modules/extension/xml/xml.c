...
static void xml_start_handler(void *data, const char *el, const char **attr UNUSED)
{
	...
	if ( ctx->value == NULL )
	{
	    ctx->value = nx_string_create_throw("<", 1);
	}
	else
	{
	    nx_string_append_throw(ctx->value, "<", 1);
	}
	nx_string_append_throw(ctx->value, el, -1);
	nx_string_append_throw(ctx->value, ">", 1);
    }
}
...
*/
	...
	if ( ctx->value == NULL )
	{
	    ctx->value = nx_string_create_throw("</", 2);
	}
	else
	{
	    nx_string_append_throw(ctx->value, "</", 2);
	}
	nx_string_append_throw(ctx->value, el, -1);
	nx_string_append_throw(ctx->value, ">", 1);
    }
...
static void xml_char_data_handler(void *data, const char *s, int len)
{
	...
	{
	    nx_string_append_throw(ctx->value, s, len);
	}
    }
}
...
nx_string_t *nx_logdata_to_xml(nx_xml_parser_ctx_t *ctx)
{
    ...
    retval = nx_string_create_throw("<Event>", 7);
	...
	{
	    nx_string_append_throw(retval, "<", 1);
	    nx_string_append_throw(retval, field->key, -1);
	    nx_string_append_throw(retval, "/>", 2);
	}
	else
	{
	    nx_string_append_throw(retval, "<", 1);
		...
		case NX_VALUE_TYPE_BOOLEAN:
		    ...
		    {
			nx_string_append_throw(retval, "TRUE", 4);
		    }
		    else
		    {
			nx_string_append_throw(retval, "FALSE", 5);
		    }
		    break;
		case NX_VALUE_TYPE_INTEGER:
		    nx_string_append_throw(retval, intstr,
		...
		case NX_VALUE_TYPE_STRING:
			...
			{
			    case '\"':
				nx_string_append_throw(retval, field->value->string->buf + start,
						 i - start);
				nx_string_append_throw(retval, "&quot;", 6);
				start = i + 1;
				break;
			    case '\'':
				nx_string_append_throw(retval, field->value->string->buf + start,
				...
				nx_string_append_throw(retval, field->value->string->buf + start,
						 i - start);
				nx_string_append_throw(retval, "&lt;", 4);
				start = i + 1;
				break;
			    case '>':
				nx_string_append_throw(retval, field->value->string->buf + start,
				...
				nx_string_append_throw(retval, field->value->string->buf + start,
						 i - start);
				nx_string_append_throw(retval, "&amp;", 5);
				start = i + 1;
				break;
			    case '\r':
				nx_string_append_throw(retval, field->value->string->buf + start,
						 i - start);
				nx_string_append_throw(retval, "&#xD;", 5);
				start = i + 1;
				break;
			    case '\n':
				nx_string_append_throw(retval, field->value->string->buf + start,
		...
		default:
		    ...
		    nx_string_append_throw(retval, value, -1);
		    free(value);
		    break;
	    }
	    nx_string_append_throw(retval, "</", 2);
