...
{  
    ...
    {
	nx_string_append_throw(ctx->tmpstr, "null,", 5);
...
static int yajl_parse_boolean_cb(void *data, int boolval)  
{
	...
	if ( boolval == 0 )
	{
	    nx_string_append_throw(ctx->tmpstr, "FALSE,", 6);
	}
	else
	{
	    nx_string_append_throw(ctx->tmpstr, "TRUE,", 5);
	}
	return ( 1 );
    }  
...
static int yajl_parse_integer_cb(void *data, long long integerval)
{
	...
	nx_string_append_throw(ctx->tmpstr, intstr, -1);
	nx_string_append_throw(ctx->tmpstr, ",", 1);
...
{  
    ...
    if ( (ctx->in_array > 0) || (ctx->in_map > 0) )
    {
	nx_string_append_throw(ctx->tmpstr, s, (int) l);
	nx_string_append_throw(ctx->tmpstr, ",", 1);
    ...
    {
	val = nx_value_new(NX_VALUE_TYPE_STRING);
	val->string = nx_string_create_throw(s, (int) l);
	nx_logdata_set_field_value(ctx->logdata, ctx->key, val);
    }
    else
    {
...
{  
    ...
    if ( (ctx->in_array > 0) || (ctx->in_map > 0) )
    {
	nx_string_append_throw(ctx->tmpstr, (const char *) stringval, (int) stringlen);
	nx_string_append_throw(ctx->tmpstr, ",", 1);
...
{  
	...
	nx_string_append_throw(ctx->tmpstr, "\"", 1);
	nx_string_append_throw(ctx->tmpstr, (const char *) stringval, (int) stringlen);
	nx_string_append_throw(ctx->tmpstr, "\":", 2);
    }
    else
    {
	if ( ctx->key != NULL )
...
boolean nx_json_is_valid(const char *json_text, apr_int64_t json_length)
{
    boolean retval;
    yajl_handle hand;
    yajl_gen g;
    ...
    json_len = (json_length < 0) ? strlen(json_text) : (apr_size_t) json_length;

    if ( (json_len < 2) || (json_text[0] != '{') || (json_text[json_len - 1] != '}') )
    {
        return FALSE;
    }
    ...
    if ( (yajl_parse(hand, (const unsigned char *) json_text, json_len) != yajl_status_ok) ||
...
nx_string_t *nx_logdata_to_json(nx_json_parser_ctx_t *ctx)
{
    ...
    retval = nx_string_create_throw((const char *) json, (int) jsonlen);
    yajl_gen_free(gen);
...
nx_string_t *nx_logdata_to_json_string_ex(nx_logdata_t *logdata)
{
    nx_json_parser_ctx_t json_parser_ctx;

    ASSERT(logdata != NULL);

    memset(&json_parser_ctx, 0, sizeof(nx_json_parser_ctx_t));
    json_parser_ctx.logdata = logdata;

    return nx_logdata_to_json(&json_parser_ctx);
}
