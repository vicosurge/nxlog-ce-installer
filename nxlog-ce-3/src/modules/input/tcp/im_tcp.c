...
static void im_tcp_accept(nx_module_t *module)
{
	...
	if ( rv == APR_SUCCESS )
	{
	    ...
	    nx_module_input_data_set(input, NX_MODULE_INPUT_CONTEXT_RECV_FROM_STR, ipstr);
...
static void im_tcp_read(nx_module_t *module, nx_event_t *event)
{
	...
	nx_module_remove_events_by_data(module, sock);
	return;
    }
    ...
    //log_debug("im_tcp_read bufstart %d buflen %d", input->bufstart, input->buflen);
    //log_dump("im_tcp_read fill", input->buf + input->bufstart, input->buflen);
    try
    {
	while ( (logdata = input->inputfunc->func(input, input->inputfunc->data)) != NULL )
	{
	    ...
	    ipstr = nx_module_input_data_get(input, NX_MODULE_INPUT_CONTEXT_RECV_FROM_STR);
    ...
    {
	//log_dump("im_tcp_read", input->buf, input->buflen);
	im_tcp_free_input(input);
...
static void im_tcp_resume(nx_module_t *module)
{
    ...
    {
	if ( imconf->connections != NULL )
	{
	    for ( input = NX_DLIST_FIRST(imconf->connections);
	          input != NULL;
	          input = NX_DLIST_NEXT(input, link) )
	    {
	        nx_module_pollset_add_socket(module, input->desc.s, APR_POLLIN | APR_POLLHUP);
            }
	}
...
static void im_tcp_event(nx_module_t *module, nx_event_t *event)
{
	...
	case NX_EVENT_POLL:
	    if ( (nx_module_get_status(module) == NX_MODULE_STATUS_RUNNING) ||
		 (nx_module_get_status(module) == NX_MODULE_STATUS_PAUSED) )
	    {
		nx_module_pollset_poll(module, TRUE);
	    }
	    break;
