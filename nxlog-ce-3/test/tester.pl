...
sub close_dst
{
    ...
    {
	# SSL_fast_shutdown makes the close() call block until the other side acknowledges
	$fh->close(SSL_fast_shutdown => 0, SSL_ctx_free => 1);
    }
    elsif ( $dst =~ /^tcp\:(.+)$/ )
    {
	$fh->shutdown(2);
...
sub exec_command
{
    ...
    elsif ( $cmd eq "SETENVVAR" ) {
	set_envvar($param);
    }
    elsif ( $cmd eq "UNSETENVVAR" ) {
	unset_envvar($param);
    }
    elsif ( $cmd eq "EXIT" )
    {
